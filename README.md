...
1. 注册或登录本地用户。
2. 进入“论文工作台”，上传一篇 PDF。
3. 点击“开始解析 PDF”，系统会保存 PDF 并生成 Markdown、图片和可检索 chunks。
4. 可下载原文 Markdown、图片 ZIP，或点击“翻译为中文 Markdown”生成译文。
5. 点击“构建论文索引”，系统会分别写入 Chroma 和 BM25。
6. 在 Ask PaperMate 区域提问，回答会展示可信引用、原文片段和 Hybrid 检索细节。
7. 在回答下方提交反馈；负面反馈自动进入 Bad Case。
8. 生成文献卡片，选择卡片库后保存。
9. 在“文献卡片库”里按卡片库、领域、年份和关键词管理卡片。
10. 在“反馈记录”页输入管理员密码，查看反馈总览、Bad Case 和原始记录。
